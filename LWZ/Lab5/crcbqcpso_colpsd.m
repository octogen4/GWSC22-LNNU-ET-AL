function outResults = crcbqcpso_colpsd(inParams,psoParams,nRuns)
%Regression of quadratic chirp using PSO
%O = CRCBQCPPSO_COLPSD(I,P,N)
...
for lpruns = 1:nRuns   
    ...
    nSamples = length(inParams.dataX);
    sampTime = inParams.dataX(end)-inParams.dataX(1);
    sampFreq = floor(nSamples/sampTime);
    dataLen = nSamples/sampFreq;
    kNyq = floor(nSamples/2)+1;
    posFreq = (0:(kNyq-1))*(1/dataLen);
    psdPosFreq = inParams.psdfun(posFreq);
    
    estSig = crcbgenqcsig_colpsd(inParams.dataX,qcCoefs);
    [estSig,~] = normsig4psd(estSig,sampFreq,psdPosFreq,1);
    estAmp = innerprodpsd(inParams.dataY,estSig,sampFreq,psdPosFreq);
    estSig = estAmp*estSig;
